...
function smartRateLimiter(req, res, next) {
    ...
    if (rateLimits[ip].length > 45) return res.status(429).json({ error: "Rate limit exceeded" });
...
app.get('/oauth/callback', async (req, res) => {
    ...
    if (!code || !state || !oauthStates[state]) return res.send(`<script>setTimeout(() => window.close(), 100);</script>`);
    ...
    try {
        ...
        oauthStates[state] = {
            status: "success",
            adminData: { userId: parseInt(userData.sub), username: userData.preferred_username || userData.name }
        };
...
app.post('/api/admin/disconnect', (req, res) => {
    if (req.body.userId && activeAdmins[req.body.userId]) delete activeAdmins[req.body.userId];
...
app.post('/api/admin/duty', verifyAdminAccess, (req, res) => {
    ...
    if (!activeAdmins[adminId]) {
        activeAdmins[adminId] = { userId: adminId, username: username || "Admin", status: "Online", serverCode: null, updatedAt: new Date().toISOString(), lastSeen: Date.now() };
    }
    ...
    if (action === "start") {
        ...
        if (!liveServers[serverCode]) return res.status(404).json({ error: "Server offline" });
    ...
    if (action === "break") {
        if (admin.status !== "on_duty") return res.status(400).json({ error: "Must be on duty" });
        admin.status = "break";
        admin.updatedAt = new Date().toISOString();
...
app.get('/api/admin/staff', verifyAdminAccess, (req, res) => {
    ...
    if (currentUserId && ALLOWED_ADMINS.includes(currentUserId)) {
        if (!activeAdmins[currentUserId]) {
            activeAdmins[currentUserId] = { userId: currentUserId, username: currentUsername || "Admin", status: "Online", serverCode: null, updatedAt: new Date().toISOString(), lastSeen: now };
        } else {
            activeAdmins[currentUserId].lastSeen = now;
            if (activeAdmins[currentUserId].status === "Offline") activeAdmins[currentUserId].status = "Online";
        }
    }

    Object.keys(activeAdmins).forEach(userId => {
        if (!ALLOWED_ADMINS.includes(parseInt(userId)) || !activeAdmins[userId].lastSeen || now - activeAdmins[userId].lastSeen > 25000) {
            delete activeAdmins[userId];
        }
    });

    let staffArr = Object.values(activeAdmins).sort((a, b) => {
...
app.post('/api/servers/:serverCode/heartbeat', (req, res) => {
    ...
    if (req.headers['authorization'] !== `Bearer ${API_TOKEN}`) return res.status(401).json({ error: "Unauthorized" });

    let teamsCounter = {};
    if (Array.isArray(playersList)) playersList.forEach(p => teamsCounter[p.team] = (teamsCounter[p.team] || 0) + 1);

    if (!liveServers[serverCode]) liveServers[serverCode] = { startTime: Date.now() };
...
app.post('/api/servers/:serverCode/commands', verifyAdminAccess, (req, res) => {
    ...
    if (action === "shutdown" && (admin.status !== "on_duty" || admin.serverCode !== serverCode)) {
        return res.status(403).json({ error: "Duty required" });
    }
...
app.post('/api/servers/:serverCode/schedule-shutdown', verifyAdminAccess, (req, res) => {
    ...
    if (!admin || admin.status !== "on_duty" || admin.serverCode !== serverCode) return res.status(403).json({ error: "Duty required" });
    if (!targetTimestamp || targetTimestamp <= Date.now()) return res.status(400).json({ error: "Invalid time" });

    const formattedTime = new Date(targetTimestamp).toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    scheduledShutdowns[serverCode] = { executeAt: targetTimestamp, formattedTime: formattedTime, senderId: senderId };
...
app.delete('/api/servers/:serverCode/schedule-shutdown', verifyAdminAccess, (req, res) => {
    if (!scheduledShutdowns[req.params.serverCode]) return res.status(404).json({ error: "No schedule found" });
    delete scheduledShutdowns[req.params.serverCode];
...
app.get('/api/servers/:serverCode/players', verifyAdminAccess, (req, res) => {
    ...
    res.json({ ...serverData, scheduledShutdown: schedule ? { time: schedule.formattedTime, timestamp: schedule.executeAt } : null });
});

app.get('/api/servers/:serverCode/players/:playerId', verifyAdminAccess, (req, res) => {
    const serverData = liveServers[req.params.serverCode];
    if (!serverData) return res.status(404).json({ error: "Server not found" });
    const player = serverData.players.find(p => String(p.userId) === String(req.params.playerId) || p.name === req.params.playerId);
...
app.get('/api/avatar/:userId', async (req, res) => {
    try {
        ...
        if (data?.data?.[0]?.state === "Completed") return res.redirect(data.data[0].imageUrl);
...
setInterval(() => {
    const now = Date.now();
    Object.keys(oauthStates).forEach(k => { if(now - oauthStates[k].time > 600000) delete oauthStates[k]; });
    ...
    Object.keys(liveServers).forEach(serverCode => {
        if (now - liveServers[serverCode].lastUpdated > 7000) {
            ...
            if (scheduledShutdowns[serverCode] && now >= scheduledShutdowns[serverCode].executeAt) {
                if (!commandsQueue[serverCode]) commandsQueue[serverCode] = [];
                commandsQueue[serverCode].push({ action: "shutdown", target: "all", reason: "Scheduled", duration: 0, senderId: scheduledShutdowns[serverCode].senderId });
                delete scheduledShutdowns[serverCode];
            }
        }
    });
